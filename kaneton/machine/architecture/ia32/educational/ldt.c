...
t_error			ia32_ldt_dump(t_ia32_ldt*		table)
{
  ...
  n.count = table->count;
  n.descriptor = (t_ia32_gdt_entry*)table->descriptor;
				       ...
				       t_uint8			clear)
{
  ...
  if (base % sizeof(t_ia32_ldt_entry))
    {
      base += sizeof(t_ia32_ldt_entry) - (base % sizeof(t_ia32_ldt_entry));
    }
  /*
   * 3)
   */

  ldt->descriptor = (t_ia32_ldt_entry*)base;
  ldt->count = entries;
  ...
  if (clear)
    {
      memset(ldt->descriptor, 0, entries * sizeof(t_ia32_ldt_entry));
    }
						 ...
						 t_uint16*	segment)
{
  ...
  if (ia32_ldt_add_segment(table, look, descriptor) != ERROR_OK)
    return ERROR_KO;
						...
						t_uint16*	selector)
{
  ...
  *selector = IA32_SEGMENT_SELECTOR_TI_LDT |
