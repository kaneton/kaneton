...
t_error			ia32_idt_dump(t_ia32_idt*		table)
{
  ...
  for (i = 0; i < table->count; i++)
    {
      if (ia32_idt_get_gate(table, i, &gate) != ERROR_OK)
	continue;
				       ...
				       t_ia32_idt*	table)
{
  ...
  if (base % sizeof(t_ia32_idt_entry))
    base += sizeof(t_ia32_idt_entry) - (base % sizeof(t_ia32_idt_entry));

  /*
   * 3)
   */

  table->descriptor = (t_ia32_idt_entry*)base;
  table->count = entries;
  ...
  if (clear)
    memset(table->descriptor, 0, entries * sizeof(t_ia32_idt_entry));
...
t_error			ia32_idt_activate(t_ia32_idt*		table)
{
  ...
  idtr.size = ia32_idt.count * sizeof(t_ia32_idt_entry);
  LIDT(idtr);
...
{
  t_ia32_idt_register	sidtr;
  t_ia32_idt_entry*	source;
  t_ia32_idt_entry*	dest;
  ...
  if (sidtr.size > table->count * sizeof(t_ia32_idt_entry))
    return ERROR_KO;
  ...
  source = (t_ia32_idt_entry*)sidtr.address;
					  ...
					  t_ia32_gate		gate)
{
  ...
  table->descriptor[index].type = IA32_DESCRIPTOR_PRESENT |
    IA32_DESCRIPTOR_SET_DPL(gate.privilege);
					  ...
					  t_ia32_gate*		gate)
{
  ...
  gate->privilege = IA32_DESCRIPTOR_GET_DPL(table->descriptor[index].type);
					     ...
					     t_uint16		gate_id)
{
  ...
  table->descriptor[gate_id].type &= ~IA32_DESCRIPTOR_PRESENT;
