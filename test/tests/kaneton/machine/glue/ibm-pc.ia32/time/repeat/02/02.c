 ...
 * license       kaneton
 *
 * file          /home/mycure/kane...ine/glue/ibm-pc.ia32/time/repeat/02/02.c
 *
 * created       julien quintard   [sun oct 17 14:37:04 2020]
 * updated       julien quintard   [sat nov 20 23:32:53 2010]
 */
...
void			test_ibmpc_ia32_time_repeat_02_handler(t_id	id,
							       t_vaddr	data)
{
  timed++;
}

void			test_ibmpc_ia32_time_repeat_02_content(void)
{
  t_uint32		start;
  i_timer		tid;
  t_uint32		flag;
  ...
  if (timer_reserve(TIMER_FUNCTION,
		    TIMER_HANDLER(test_ibmpc_ia32_time_repeat_02_handler),
		    0,
		    300,
  ...
  timer_repeat(tid, TIMER_REPEAT_ENABLE);
  ...
  if (timed != 3)
    TEST_ERROR("the timer has not been triggered thrice as expected\n");

  while (1)
    ;
}

void			test_ibmpc_ia32_time_repeat_02(void)
{
  i_thread		thread;
  o_thread*		o;
  ...
  ctx.pc = (t_vaddr)test_ibmpc_ia32_time_repeat_02_content;
