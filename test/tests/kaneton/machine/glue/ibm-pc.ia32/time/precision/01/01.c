 ...
 * license       kaneton
 *
 * file          /home/mycure/kane.../glue/ibm-pc.ia32/time/precision/01/01.c
 *
 * created       julien quintard   [sun oct 17 14:37:01 2010]
 * updated       julien quintard   [sat nov 20 23:42:08 2010]
 */
...
static volatile i_timer		ticks = 0;

/*
 * ---------- test ------------------------------------------------------------
 */
 
void			test_ibmpc_ia32_time_precision_01_handler(t_id	  id,
								  t_vaddr data)
{
  timed = 1;

  ticks = test_ibmpc_ia32_time_cmos();
}

void			test_ibmpc_ia32_time_precision_01_content(void)
{
  t_uint32		start;
  i_timer		tid;
  ...
  if (timer_reserve(TIMER_FUNCTION,
		    TIMER_HANDLER(test_ibmpc_ia32_time_precision_01_handler),
		    0,
		    500,
...
void			test_ibmpc_ia32_time_precision_01(void)
{
  i_thread		thread;
  o_thread*		o;
  ...
  ctx.pc = (t_vaddr)test_ibmpc_ia32_time_precision_01_content;
