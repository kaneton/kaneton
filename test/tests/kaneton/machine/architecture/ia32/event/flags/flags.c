 ...
 * updated       julien quintard   [sun nov 21 00:07:24 2010]
 */
...
void			test_ia32_event_flags_handler(t_id		id)
{
  t_uint32		eflags;
...
void			test_ia32_event_flags_content(void)
{
  t_uint32		eflags;

  if (event_reserve(3,
		    EVENT_FUNCTION,
		    EVENT_HANDLER(test_ia32_event_flags_handler),
		    0) != ERROR_NONE)
...
void			test_ia32_event_flags(void)
{
  i_thread		thread;
  o_thread*		o;
  ...
  ctx.pc = (t_vaddr)test_ia32_event_flags_content;
