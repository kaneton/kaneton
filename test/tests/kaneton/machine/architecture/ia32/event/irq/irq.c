 ...
 * updated       julien quintard   [sun nov 21 00:11:17 2010]
 */
...
void			test_ia32_event_irq_handler(t_id		id)
{
  thrown++;
}

void			test_ia32_event_irq_content(void)
{
  int			i;

  if (event_reserve(32 + 6,
		    EVENT_FUNCTION,
		    EVENT_HANDLER(test_ia32_event_irq_handler),
		    0) != ERROR_NONE)
...
void			test_ia32_event_irq(void)
{
  ...
  ctx.pc = (t_vaddr)test_ia32_event_irq_content;
