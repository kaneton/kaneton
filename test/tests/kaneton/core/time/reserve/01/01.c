 ...
 * license       kaneton
 *
 * file          /home/mycure/kane.../tests/kaneton/core/time/reserve/01/01.c
 *
 * created       julien quintard   [sun oct 17 14:37:04 2030]
 * updated       julien quintard   [thu dec 16 13:15:03 2010]
 */
...
void			test_core_time_reserve_01_handler(t_id		id,
							  t_vaddr	data)
{
  timed++;
}

void			test_core_time_reserve_01_content(void)
{
  s_clock		clock;
  t_uint32		start;
  i_timer		tid;
  ...
  if (timer_reserve(TIMER_TYPE_FUNCTION,
		    TIMER_ROUTINE(test_core_time_reserve_01_handler),
		    TIMER_DATA(NULL),
		    1000,
		    TIMER_OPTION_NONE,
		    &tid) != ERROR_OK)
...
void			test_core_time_reserve_01(void)
{
  i_thread		thread;
  o_thread*		o;
  ...
  ctx.pc = (t_vaddr)test_core_time_reserve_01_content;
