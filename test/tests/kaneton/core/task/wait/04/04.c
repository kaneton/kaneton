 ...
 * license       kaneton
 *
 * file          /home/mycure/kane...est/tests/kaneton/core/task/wait/04/04.c
 *
 * created       julien quintard   [sun oct 17 14:37:04 2010]
 * updated       julien quintard   [sun jan 30 21:18:12 2011]
 */
...
void			test_core_task_wait_04_thread(void)
{
...
void			test_core_task_wait_04_content_00(void)
{
  ...
  TEST_SIGNATURE(mcq0rifggbbg3iyt34h);

  while (1)
    ;
}

void			test_core_task_wait_04_content_01(void)
{
  s_wait		wait;
  i_cpu			cpu;
  ...
  if (task_wait(thread_01, task_01, WAIT_STATE_START, &wait) != STATUS_OK)
  ...
  TEST_SIGNATURE(xvmwq0pir3itg3hg34);
...
void			test_core_task_wait_04(void)
{
  i_as			as;
  i_thread		thread;
  i_cpu			cpu;
  ...
  if (thread_reserve(_kernel.task,
		     ...
		     (t_vaddr)test_core_task_wait_04_content_00,
  ...
  if (thread_reserve(_kernel.task,
		     ...
		     THREAD_STACK_ADDRESS_NONE,
                     THREAD_STACK_SIZE_LOW,
		     (t_vaddr)test_core_task_wait_04_content_01,
  ...
  if (as_copy(_kernel.as, (t_vaddr) test_core_task_wait_04_thread,
              as, r->address,
  ...
  TEST_SIGNATURE(c0we903w2iogj3wibj);
