...
void			test_core_task_sleep_content(void)
{
  ...
  if (task_sleep(task_01, 5000) != ERROR_OK)
  ...
  if (thread_sleep(thread_00, 4000) != ERROR_OK)
    TEST_HANG("[thread_sleep] error");
...
void			test_core_task_sleep(void)
{
  ...
  ctx.pc = (t_vaddr)test_core_task_sleep_content;
  ...
  ctx.sp = o->stack + o->stacksz - 16;
  ctx.pc = (t_vaddr)test_core_task_sleep_thread;
