 ...
 * last update   Thu Aug 25 19:20:18 2005   mycure
 */

/*
 * copyright EPITA System Laboratory
 *
...
#define BPT_NODESZ_T		unsigned int
...
#define		bpt_init(T, _bpt_, _nodesz_, _uaddr_, _ukey_, _uval_,         \
                         _flags_, _load_, _unload_, _addrcmp_, _keycmp_,      \
                         _valcmp_, _callback_, _data_, _unused_)              \
  bpt_init_##T(_bpt_, _nodesz_, _uaddr_, _ukey_, _uval_, _flags_, _load_,     \
               _unload_, _addrcmp_, _keycmp_, _valcmp_, _callback_,           \
               _data_, _unused_)
...
#define		bpt_make_types(T, _t_bpt_nodesz_, _t_bpt_ndi_, _t_bpt_uni_,   \
                               _t_bpt_nodes_, _t_bpt_height_, _t_bpt_addr_,   \
                               _t_bpt_key_, _t_bpt_value_, _t_bpt_inentry_,   \
                               _t_bpt_lfentry_)                               \
                                                                              \
typedef struct s_bpt_##T	t_bpt_##T;                                    \
                                                                              \
typedef _t_bpt_nodesz_		t_bpt_nodesz_##T;                             \
...
{                                                                             \
  ...
  if (opts & BPT_OPT_TREE)                                                    \
	...
	{                                                                     \
	  BPT_LOAD(bpt, &node, current.node);                                 \
                                                                              \
	  if (BPT_GET_HEAD(T, &node, prv) == bpt->uaddr)                      \
	    {                                                                 \
	      BPT_UNLOAD(bpt, &node);                                         \
	      return (-1);                                                    \
	    }                                                                 \
...
{                                                                             \
  ...
  if (opts & BPT_OPT_TREE)                                                    \
	...
	{                                                                     \
	  BPT_LOAD(bpt, &node, current.node);                                 \
                                                                              \
	  if (BPT_GET_HEAD(T, &node, nxt) == bpt->uaddr)                      \
	    {                                                                 \
	      BPT_UNLOAD(bpt, &node);                                         \
	      return (-1);                                                    \
	    }                                                                 \
  ...
  if (opts & BPT_OPT_PARENT)                                                  \
    {                                                                         \
      ...
      opts &= ~BPT_OPT_PARENT;                                                \
                                                                              ...
                                                                              \
  if (BPT_GET_HEAD(T, node, parent) == bpt->uaddr)                            \
    return (0);                                                               \
                                                                              \
...
{                                                                             \
  ...
  return (-1);                                                                \
                                                                              ...
                                                                              \
/*                                                                            \
 * this function checks whether a full collision (key:value) exists in a leaf \
 */                                                                           \
                                                                              \
int			bpt_check_collide_##T(t_bpt(T)		*bpt,         \
					      t_bpt_imm(T)	*node1,       \
...
{                                                                             \
  ...
  t_bpt_ndi(T)		ndi2;                                                 \
                                                                              \
  bpt_insert_sort(T, bpt, node1, BPT_OPT_NOKEY(T), &ndi1, BPT_OPT_TAIL);      \
                                                                              \
...
{                                                                             \
  ...
  t_bpt_ndi(T)		ndi2;                                                 \
                                                                              \
  bpt_insert_sort(T, bpt, node2, BPT_OPT_NOKEY(T), &ndi2, BPT_OPT_HEAD);      \
                                                                              \
...
#define		bpt_make(T, _t_bpt_nodesz_, _t_bpt_ndi_, _t_bpt_uni_,         \
                         _t_bpt_nodes_, _t_bpt_height_, _t_bpt_addr_,         \
                         _t_bpt_key_, _t_bpt_value_, _t_bpt_inentry_,         \
                         _t_bpt_lfentry_, _key_, _value_)                     \
  bpt_make_types(T, _t_bpt_nodesz_, _t_bpt_ndi_, _t_bpt_uni_, _t_bpt_nodes_,  \
                 _t_bpt_height_, _t_bpt_addr_, _t_bpt_key_, _t_bpt_value_,    \
                 _t_bpt_inentry_, _t_bpt_lfentry_)                            \
  bpt_make_protos(T)                                                          \
