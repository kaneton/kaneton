...
def		parse(p):
  ...
  p.structures, p.contents = structures.parse(p.contents)

  debug(p, DEBUG_STRUCTURES)

  p.enumerations, p.contents = enumerations.parse(p.contents)

  debug(p, DEBUG_ENUMERATIONS)

  p.unions, p.contents = unions.parse(p.contents)

  debug(p, DEBUG_UNIONS)
