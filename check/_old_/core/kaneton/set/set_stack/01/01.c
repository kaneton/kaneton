 ...
 * project       kaneton
 *
 * file          /home/rhino/kaneton/check/core/kaneton/set/set_stack/01/01.c
 *
...
/*
 * create a stack set with no option and release it. nothing more.
 */
...
void		check_set_stack_01(void)
{
  ...
  if (set_reserve(stack, SET_OPT_ALLOC | SET_OPT_FREE,
		  sizeof(t_id), &id) == ERROR_NONE)
    printf("error: set_reserve() accepted bad arguments\n");

  if (set_reserve(stack, SET_OPT_ORGANISE,
		  sizeof(t_id), &id) == ERROR_NONE)
    printf("error: set_reserve() accepted bad arguments\n");
