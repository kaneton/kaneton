 ...
 * project       kaneton
 *
 * file          /home/rhino/kaneton/check/core/kaneton/set/set_pipe/03/03.c
 *
...
/*
 * create a pipe with ALLOC/FREE options then push some data.
 */
...
void		check_set_pipe_03(void)
{
  ...
  if (set_reserve(pipe, SET_OPT_ALLOC, sizeof(t_obj), &id) != ERROR_NONE)
  ...
  if (set_reserve(pipe, SET_OPT_FREE, sizeof(t_obj), &id) != ERROR_NONE)
    printf("error: set_reserve()\n");
