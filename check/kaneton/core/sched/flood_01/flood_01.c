...
void		check_sched_flood_01_entry(void)
{
  t_id		id;
  t_uint32	start;
  int		i;
  ...
  while ((start + 3) % 60 != check_cmos_sec() && executed != 25)
    ;

  ASSERT(executed == 25, "One or more threads not executed\n")

  TEST_LEAVE();
}

CHECK_WITH_THREAD(check_sched_flood_01)
