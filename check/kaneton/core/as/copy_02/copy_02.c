...
void		check_as_copy_02(void)
{
  ...
  ASSERT(region_reserve(as1, seg1, 0, REGION_OPT_FORCE,
			0x20000000, PAGESZ, &reg) == ERROR_NONE,
	 "error reserving region\n");
  ASSERT(region_reserve(as1, seg2, 0, REGION_OPT_FORCE,
			0x20001000, PAGESZ, &reg) == ERROR_NONE,
	 "error reserving region\n");
  ASSERT(region_reserve(as1, seg3, PAGESZ, REGION_OPT_FORCE,
			0x20002000, 2 * PAGESZ, &reg) == ERROR_NONE,
	 "error reserving region\n");
  ...
  ASSERT(region_reserve(as2, seg4, 0, REGION_OPT_FORCE,
			0x40000000, 2 * PAGESZ, &reg) == ERROR_NONE,
	 "error reserving region\n");
  ASSERT(region_reserve(as2, seg5, PAGESZ, REGION_OPT_FORCE,
			0x40002000, PAGESZ, &reg) == ERROR_NONE,
	 "error reserving region\n");
  ASSERT(region_reserve(as2, seg6, 0, REGION_OPT_FORCE,
			0x40003000, PAGESZ, &reg) == ERROR_NONE,
	 "error reserving region\n");
  ...
  ASSERT(as_copy(as1, 0x20000100, as2, 0x40000100, 4 * PAGESZ - 0x200) == ERROR_NONE,
	 "error in as_copy\n");
