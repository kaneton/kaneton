#include <klibc.h>
#include <kaneton.h>
#include "../common/common.h"

void		check_segment_perms_02(void)
{
  i_task	task;
  o_segment*	o;
  t_asid	as;
  t_segid	seg;


  TEST_ENTER();

  TEST_NEW_AS(task, as);

  ASSERT(segment_reserve(as, PAGESZ, PERM_READ | PERM_WRITE, &seg) == ERROR_NONE,
         "Error reserving segment\n");
  ASSERT(segment_get(seg, &o) == ERROR_NONE, "Error getting segment descriptor structure\n");
  ASSERT(o->perms == (PERM_READ | PERM_WRITE), "Bad permission field after initialisation\n");

  ASSERT(segment_perms(seg, PERM_READ) == ERROR_NONE,
	 "Error changing segment perms\n");
  ASSERT(segment_perms(seg, PERM_READ | PERM_EXEC) == ERROR_NONE,
	 "Error changing segment perms\n");
  ASSERT(o->perms == (PERM_READ | PERM_EXEC), "Bad permission field after perms modification\n");

  ASSERT(segment_release(seg) == ERROR_NONE, "error releasing segment\n");

  TEST_LEAVE_AS(task, as);

  TEST_LEAVE();
}
