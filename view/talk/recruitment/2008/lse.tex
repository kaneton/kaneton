...
% license       kaneton
%
% file          /home/mycure/kaneton/view/talk/recruitment/2008/lse.tex
%
% created       matthieu bucchianeri   [fri nov  2 14:14:33 2007]
% updated       julien quintard   [fri may 23 20:14:00 2008]
%
...
\begin{frame}
  ...
  \begin{itemize}
  \item
    Kernels, Operating systems and low-level programming;
  \item
  ...
  The whole course is given to about 60 students every year.
...
\begin{frame}
  ...
  popular, with about 60 students the last two years.
\end{frame}
...
\begin{frame}
  \frametitle{kaneton people}
	...
	\item
	Responsible of kaneton courses from 2004 to 2006
        \item
	...
	\item
	Responsibles of the kaneton course in 2007
	\item
  ...
  \begin{itemize}
    \item
      Mathieu Sélariès: Assistant project manager and occasional developer
    \item
  ...
  \begin{itemize}
    ...
    \item
      Jean-Pascal Billaud: kaneton courses for 2004 and 2005
  \end{itemize}
...
\begin{frame}
  ...
  In OSes like Linux or *BSD, drivers are included in the kernel, and
...
\begin{frame}
  ...
  All non critical code (drivers \& services) is pushed out of the
...
\begin{frame}
  ...
  and application's request goes through messages.
...
\begin{frame}
  ...
  tasks. This protection mechanism is achieved through virtual memory.
  ...
  A task is made of one or more threads. Threads are implemented on
...
\begin{frame}
  ...
  them, a special mechanism called ``sets'' is used for containing those
  objects.

  \-

  Sets implement different common containers (list, stack,
  FIFO\ldots), so the kernel code for manipulating objects is unified and
  clear.
...
\begin{frame}
  ...
  permissions on this object. A security mechanism based on
...
\begin{frame}
  ...
  or source files. Interacting with them means issuing a simple function or
  system call.
...
\begin{frame}
  ...
  In addition, this testsuite is used to evalutate the ing2 projects
  and to give grades.
  ...
  almost every feature of the kernel.
...
\begin{frame}
  ...
  \item
    Presentation of the kaneton tools and environment
  \item
...
\begin{frame}
  ...
  kaneton provides its own interface, completely different from \emph{UNIX}.

  \-

  \emph{POSIX} is an interface which was introduced in order to
  normalise the \emph{C Library} layer.

  \-

  This project consists in implementing a \emph{POSIX} compliant layer
  that translates \emph{POSIX} functionalities into kaneton interface
  calls.

  \-

  The student will have to study very precisely the \textit{POSIX}
  standard as well as the kaneton interface in order to develop the
  emulation library.

  \-

  Since \textit{UNIX} highly relies on the concept of \textit{files},
  it is very likely that the student will have to port or develop a
  hard-disk driver as well as a basic file system.
...
\begin{frame}
  ...
  \begin{itemize}[<+->]
    \item
    The emulation layer: \emph{libposix}
    \item
    A Virtual Filesystem Switch (VFS), plus one or more usable block
    device drivers and filesystems
    \item
    A few test application taken from \emph{UNIX} systems
    \item
    Optionally, an emulation service to run applications without
    recompiling it
  \end{itemize}
...
\begin{frame}
  ...
  This project consists in porting kaneton to a new microprocessor
  architecture, in this case, \textit{AMD-64}.

  \-

  This architecture will probably become more and more used soon, so
  it is a good idea to support it in kaneton.

  \-

  It is interesting to port kaneton to AMD-64 since this architecture
  is 64-bit and its implementation could reveal design mistakes and/or
  implementation bugs in the generic part of the microkernel.

  \-

  In addition, having kaneton running on two different platforms would mean
  that the kaneton portability scheme is correct.

  \-

  The student will have to study a complete new architecture by reading
  the microprocessor specifications.
...
\begin{frame}
  ...
  \begin{itemize}[<+->]
    \item
    The bootloader ported to AMD-64
    \item
    The machine-dependent code for AMD-64
    \item
    Some new specific tests for this architecture
    \item
    A port of a few basic services if needed
  \end{itemize}
...
\begin{frame}
  ...
  This project consists in designing and implementing a complete
  security model composed of some in-kernel primitives as well as at
  the operating system level.

  \-

  For now, the kaneton microkernel provides the concept of
  \textit{capabilities}. The student will have to complete the current
  implementation in order to move to the study of the security in the
  operating system, even its in distributed form.

  \-

  A possibly suitable access control scheme could be the RBAC -
  Role-Based Access Control. The student will have, once again, to
  confirm this possibility via a preliminary study and then implement
  the services necessary to this scheme so that applications can use
  it.

  \-

  The student will have to implement multi-users environment for
  kaneton, completing the work of the POSIX project.
...
\begin{frame}
  ...
  \begin{itemize}[<+->]
    \item
    A complete description of the chosen solutions
    \item
    One or more services implementing your security model
    \item
    A service implementing \emph{POSIX} multi-users environment
    \item
    A set of tests showing the efficiency of your solution
  \end{itemize}
...
\begin{frame}
  ...
  This project consists in developing a \textit{DHT} --- Distributed
  Hash Table, crucial for distributed applications as they need to
  store data and make them accessible to the other nodes of the
  distributed system.

  \-

  This \textit{DHT} will be heavily used in kayou, especially for the
  distributed file system, but could also be used by many distributed
  applications.

  \-

  The student will have to briefly study this research field through
  the well-known papers and projects and then to design and finally
  implement the complete \textit{DHT}.

  \-

  This project is clearly research-oriented and many firms are highly
  interested in such a performant and secure implementation in order
...
\begin{frame}
  ...
  \begin{itemize}[<+->]
    \item
    An implementation of DHT usable in kayou
    \item
    A full documentation
    \item
    A testsuite and/or benchmarks of your DHT
    \item
    Optionally, a paper about your DHT implementation
  \end{itemize}
...
\begin{frame}
  ...
  by grades from kaneton supervisors.
...
\begin{frame}
  ...
  Recruits will have to appropriate to themselves the currently existing
  ...
  to communicate with the EPITA administration (planning, grades\ldots).

  \-

  In ing3, your work is to ensure the project continuation at EPITA
