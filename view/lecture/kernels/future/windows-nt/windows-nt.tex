    ...
    \item
      ...
      \begin{itemize}
        ...
        \item
          translates ``//./C:/foo'' to ``/GLOBAL??/C:/foo''
        \item
          gets a \code{DEVICE\_OBJECT} on ``/Device/HarddiskVolume0''
        \item
          sends an \code{IRP\_MJ\_CREATE} irp to io manager for the device object,  with ``/foo'' as argument
      \end{itemize}
...
\begin{frame}[containsverbatim]
  ...
  \begin{itemize}
    \item
      \code{HANDLE CreateEvent(\textbf{LPSECURITY\_ATTRIBUTES}, ...);}
    \item
      \code{HANDLE CreateMutex(\textbf{LPSECURITY\_ATTRIBUTES}, ...);}
    \item
      \code{HANDLE CreateFile(... , \textbf{LPSECURITY\_ATTRIBUTES}, ...);}
    \item
      \code{HANDLE CreateThread(\textbf{LPSECURITY\_ATTRIBUTES}, ...);}
  \end{itemize}
