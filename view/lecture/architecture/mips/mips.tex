...
% updated       julien quintard   [wed jun  4 15:14:52 2008]
%
...
\pgfdeclareimage[interpolate=true,width=188pt]
                {philosophies}
		{figures/philosophies}

\pgfdeclareimage[interpolate=true,height=26pt]
                {r-format}
		{figures/r-format}
\pgfdeclareimage[interpolate=true,height=26pt]
                {i-format}
...
\pgfdeclareimage[interpolate=true,height=46pt]
                {overflow-handling}
...
\pgfdeclareimage[interpolate=true,height=125pt]
                {memory-access}
		{figures/memory-access}

\pgfdeclareimage[interpolate=true,height=79pt]
                {jump-instruction}
		{figures/jump-instruction}

\pgfdeclareimage[interpolate=true,height=45pt]
                {amdhal-rule}
		{figures/amdhal-rule}

\pgfdeclareimage[interpolate=true,height=129pt]
                {endianness}
		{figures/endianness}
\pgfdeclareimage[interpolate=true,height=129pt]
                {memory-layout}
...
\pgfdeclareimage[interpolate=true,height=92pt]
                {pipeline-overview}
		{figures/pipeline-overview}
\pgfdeclareimage[interpolate=true,height=90pt]
                {pipeline-balancing}
		{figures/pipeline-balancing}
\pgfdeclareimage[interpolate=true,height=105pt]
                {moore-law}
		{figures/moore-law}
\pgfdeclareimage[interpolate=true,height=120pt]
                {pipeline-views}
		{figures/pipeline-views}
\pgfdeclareimage[interpolate=true,height=150pt]
                {detailed-pipeline}
		{figures/detailed-pipeline}
\pgfdeclareimage[interpolate=true,height=68pt]
                {next-instruction-stream}
		{figures/next-instruction-stream}
\pgfdeclareimage[interpolate=true,height=134pt]
                {decode-branch}
		{figures/decode-branch}
\pgfdeclareimage[interpolate=true,height=91pt]
                {decode-multiplexer}
		{figures/decode-multiplexer}
\pgfdeclareimage[interpolate=true,height=86pt]
                {comparison-decode-execute-1}
		{figures/comparison-decode-execute-1}
\pgfdeclareimage[interpolate=true,height=116pt]
                {comparison-decode-execute-2}
		{figures/comparison-decode-execute-2}
\pgfdeclareimage[interpolate=true,height=101pt]
                {comparison-decode-execute-3}
		{figures/comparison-decode-execute-3}
\pgfdeclareimage[interpolate=true,height=144pt]
                {decode-stage}
		{figures/decode-stage}

\pgfdeclareimage[interpolate=true,height=61pt]
                {dependency-problem-overview}
		{figures/dependency-problem-overview}
\pgfdeclareimage[interpolate=true,height=56pt]
                {dependency-solution-overview}
		{figures/dependency-solution-overview}
\pgfdeclareimage[interpolate=true,height=151pt]
                {dependency-detailed-pipeline}
		{figures/dependency-detailed-pipeline}
\pgfdeclareimage[interpolate=true,height=81pt]
                {bypasses}
		{figures/bypasses}
\pgfdeclareimage[interpolate=true,height=141pt]
                {example-pipeline}
		{figures/example-pipeline}
\pgfdeclareimage[interpolate=true,height=150pt]
                {strlen-part-1}
		{figures/strlen-part-1}
\pgfdeclareimage[interpolate=true,height=139pt]
                {strlen-part-2}
		{figures/strlen-part-2}
\pgfdeclareimage[interpolate=true,height=128pt]
                {strlen-part-3}
		{figures/strlen-part-3}
\pgfdeclareimage[interpolate=true,height=108pt]
                {strlen-part-4}
		{figures/strlen-part-4}

\pgfdeclareimage[interpolate=true,height=103pt]
                {software-pipeline}
		{figures/software-pipeline}

\pgfdeclareimage[interpolate=true,height=135pt]
                {superpipeline-views}
		{figures/superpipeline-views}
\pgfdeclareimage[interpolate=true,height=165pt]
                {detailed-superpipeline}
		{figures/detailed-superpipeline}
\pgfdeclareimage[interpolate=true,height=101pt]
                {superpipeline-bypasses}
		{figures/superpipeline-bypasses}
\pgfdeclareimage[interpolate=true,height=41pt]
...
\pgfdeclareimage[interpolate=true,height=137pt]
                {superscalar-example}
		{figures/superscalar-example}
\pgfdeclareimage[interpolate=true,height=152pt]
                {superscalar-correction}
		{figures/superscalar-correction}

\pgfdeclareimage[interpolate=true,height=61pt]
                {jointure-overview}
		{figures/jointure-overview}
\pgfdeclareimage[interpolate=true,height=121pt]
                {jointure-communication}
		{figures/jointure-communication}
\pgfdeclareimage[interpolate=true,height=130pt]
                {jointure-example}
		{figures/jointure-example}
\pgfdeclareimage[interpolate=true,height=125pt]
                {memory-organisation}
		{figures/memory-organisation}
\pgfdeclareimage[interpolate=true,height=60pt]
                {memory-access-time}
		{figures/memory-access-time}
\pgfdeclareimage[interpolate=true,height=102pt]
                {spatial-locality}
		{figures/spatial-locality}
\pgfdeclareimage[interpolate=true,height=121pt]
                {cache-hierarchy}
		{figures/cache-hierarchy}
\pgfdeclareimage[interpolate=true,height=112pt]
                {cache-overview}
		{figures/cache-overview}
\pgfdeclareimage[interpolate=true,height=111pt]
                {jointure-cache}
		{figures/jointure-cache}
\pgfdeclareimage[interpolate=true,height=93pt]
                {natural-partitionning}
		{figures/natural-partitionning}
\pgfdeclareimage[interpolate=true,height=111pt]
                {cache-example}
		{figures/cache-example}
\pgfdeclareimage[interpolate=true,height=127pt]
                {cache-internals}
		{figures/cache-internals}
\pgfdeclareimage[interpolate=true,height=186pt]
                {set-associative-overview}
		{figures/set-associative-overview}
\pgfdeclareimage[interpolate=true,height=147pt]
                {set-associative-internals}
		{figures/set-associative-internals}
\pgfdeclareimage[interpolate=true,height=80pt]
                {temporal-locality}
		{figures/temporal-locality}
\pgfdeclareimage[interpolate=true,height=151pt]
                {snoopy}
		{figures/snoopy}
\pgfdeclareimage[interpolate=true,height=148pt]
                {cache-automata}
...
\begin{frame}[containsverbatim]
  ...
  \begin{verbatim}
    void vector(int* a, int* b, int *c, unsigned int size)
    {
      for (i = 0; i < size; i++)
        c[i] = 2 * a[i] + 3 * b[i];
    }
...
\begin{frame}
  ...
  You could notice that the CPI increases while we try to optimize the
  assembly code.

  \-

  Indeed, although we managed to reduce the number of cycles, the loop cannot
  be further optimised as every time an instruction was removed, a stall
  was introduced, keeping the most optimised CPIu at $\frac{5}{3}$.
\end{frame}
...
\begin{frame}
  ...
  \begin{enumerate}
    \item
      \textbf{R}ead: first, an element of an array for example is read from
      the memory.
    \item
      \textbf{O}peration: then, an operation is performed on this element.
    \item
      \textbf{W}rite: finally, the computed value is stored back into memory.
  \end{enumerate}
\end{frame}
...
\begin{frame}
  ...
  \begin{center}
    \pgfuseimage{jointure-communication}
  \end{center}
\end{frame}
...
\begin{frame}
  ...
  \begin{itemize}
    \item
      ...
      99.99999\% of memory accesses --- \textit{i.e} not in cache ---
      are in main memory.
    \item
...
\begin{frame}
  ...
  Locality so cache implementations should not base everything on the
  Temporal Locality.
...
\begin{frame}
  ...
  \begin{itemize}
    ...
    \item
      ...
      This property is very cumbersome for multiprocessors.
  \end{itemize}
\end{frame}
...
\begin{frame}
  ...
  waiting for Invalidation Signals. This automata is called the
  \textbf{Snoopy}, from the famous dog.
  ...
  \begin{itemize}
    \item
      ...
      An invalidation signal is sent to all caches which snoop and
    ...
    \item
      ...
      \begin{itemize}
        ...
        \item
          ...
          Request other caches to update copy in main memory, then snoop
          that value from the bus.
      \end{itemize}
  \end{itemize}

  \-

  Note that a separate invalidation bus called \textit{Snoopy Bus} can be used
  for write traffic.
\end{frame}
...
\begin{frame}
  ...
  due to the inconsistent memory especially as the number of processor
  increase.

  \-

  Therefore researchers tried to optimise the Write Through Policy.
\end{frame}
...
\begin{frame}
  ...
  Everytime a block is overwritten, the block is pushed into the Store Buffer.

  \-

  Then the cache automata's role is to get a block from the Store Buffer
...
\begin{frame}
  ...
  Consider a MIPS microprocessor with a five-stage pipeline. We want to
  study the memory performances impact on the whole microprocessor.

  \-

  First, load-like instructions represent $20\%$ of the executed
  instructions while store-like instructions represent $10\%$ of them.

  \-

  We are going to study a concrete system by exhibiting the penalty impacted
  over a perfect system.

  \-

  \begin{itemize}
    \item
      \textbf{Perfect System}

      \-

      Readings and writings take one cycle whie the CPI is equal to
      $1.1$ cycle per instruction.
    \item
      \textbf{Concrete System}

      A memory reading takes $10$ cycles while a writing takes $4$ cycles.
  \end{itemize}
\end{frame}
...
\begin{frame}
  ...
  On a perfect system a writing takes one cycle while in a concrete system a
  writing takes 4 cycles; the writing penalty is therefore $3$ cycles per
  instruction.
  ...
  The writing penalty is therefore equal to:

  \begin{eqnarray*}
    WP & = & \frac{penalty}{writing} \times probability \\
...
\begin{frame}
  \frametitle{Reading Probabilities}

  We can easily compute the number of reading instructions and their
  probabilities:
...
\begin{frame}
  ...
  a reading takes 10 cycles. The reading penalty is thus $9$ cycles
  per instruction.
  ...
  \begin{eqnarray*}
    RP & = & \frac{penalty}{reading} \times probability \\
...
\begin{frame}
  ...
  Let's consider a $64$ Mbytes memory with a two-level set associative cache
  of degree two.

  \-

  The cache is unified and can contain $16$ Kbytes of data while the memory
...
\begin{frame}
  ...
  \textit{What is the usefulness of the set associative cache policies
    compared to direct-mapping ones?}
\end{frame}
...
\begin{frame}
  ...
  Let's assume a Write Through caching policy with $16$-byte cache lines.

  \-

  The system relies on a $32$-bit bus where masters generally take $9$ cycles
  for acquiring the bus and one cycle for transferring $4$ bytes.
\end{frame}
...
\begin{frame}
  ...
  \textit{How much time would a Lw miss take?}

  \-

  \textit{How much time would a Sw miss take?}

  \-

  \textit{How much time would a write operation take with a Store Buffer?}
\end{frame}
...
\begin{frame}
  ...
  Since the bus is pipelined, it takes one cycle for requesting the memory
  which ends up with a cache miss and nine cycles for acquiring the bus.

  \-

  Then, a read needs to load a whole block and thereofre takes one cycle for
  the first address, four cycles for the $16$ bytes of data, one cycle to
  update the cache and one cycle for responding to the processor:

  \begin{center}
    $1 + 9 + 1 + \frac{16}{4} + 1 + 1 = 17$ cycles
  \end{center}

  On the other hand, a write takes one cycle for the first address, one
  cycle for the $4$-byte data and one cycle for responding to the processor:

  \begin{center}
    $1 + 9 + 1 + \frac{4}{4} + 1 = 13$ cycles
  \end{center}

  With a Store Buffer, write operations would take a single cycle.
\end{frame}
