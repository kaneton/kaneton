...
%% updated       julien quintard   [sat apr 15 22:12:10 2006]
%%
...
\function{sched\_next}{(i\_thread* \argument{thread})}
	 {
	   This function returns in \argument{thread} the identifer
	   of the next thread to execute.
	 }

\function{sched\_switch}{(i\_thread \argument{thread})}
	 {
...
\begin{comment}
       ...
       int MPI_Send(void *buf, int count, MPI_Datatype dtype, int dest,
        ...
        -> message_send(FLAG_ASYNC | FLAG_BLOCK)

       int MPI_Ssend(void *buf, int count, MPI_Datatype dtype,
                     int dest, int tag, MPI_Comm comm)
        -> message_send(FLAG_SYNC | FLAG_BLOCK) // transmit(), post()
                                                // pass()
          -> message_transmit();
       ...
       int MPI_Isend(void *buf, int count, MPI_Datatype dtype,
                     int dest, int tag, MPI_Comm comm,
                     MPI_Request *req)
        -> message_send(FLAG_ASYNC | FLAG_NBLOCK, &request)
          -> message_throw();

       int MPI_Issend(void *buf, int count, MPI_Datatype dtype,
                      int dest, int tag, MPI_Comm comm,
                      MPI_Request *req)
        -> message_send(FLAG_SYNC | FLAG_NBLOCK, &request) // hurl(), emit()
                                                           // cast()
          -> message_post();
...
---
       ...
       int MPI_Recv(void *buf, int count, MPI_Datatype dtype,
        ...
        -> message_receive(FLAG_BLOCK)

       int MPI_Irecv(void *buf, int count, MPI_Datatype dtype,
                     int src, int tag, MPI_Comm comm,
                     MPI_Request *req)
        -> message_receive(FLAG_NBLOCK) // get(), acquire(), obtain()
                                        // pick(), catch()
          -> message_grab();
