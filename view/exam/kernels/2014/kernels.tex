...
\begin{parts}
  ...
  \begin{subparts}
    ...
    % q
    \subpart[2] Quelle est l'interface (control-flow ou data-flow ou contrat) firmware/bootloader sur les ordinateurs IBM-PC?\\
...
\begin{parts}
  ...
  % q
  \part[2] La fonction de traitement des interruptions d'un ordinateur requiert 2 ms (temps de commutation des processus inclus) par top d'horloge. La fréquence de l'horloge est de 60 Hz. Quel est le pourcentage de temps que le processeur consacre à l'horloge? Commenter.

  % q
  \part[2] Pourquoi le top d'horloge (tick) d'un système général est particulièrement adaptée au mode "edge-triggered"?
...
\begin{parts}
  ...
  \begin{subparts}
    ...
    \subpart[2] d'utiliser la technique de "copy-on-write" après une fork.
  \end{subparts}
...
\begin{parts}
  ...
  % q
  \bonuspart[1] Les hyperviseurs communéments utilisées aujourd'hui (vmware, xen...) sont dit hybrides. Expliquer en quoi le sont-ils avec des exemples concrêts de ce mode hybride.
