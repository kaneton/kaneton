...
\begin{verbatim}
t_error       mipsr3000_region_release(i_as asid, i_region regid)
{
  o_as*       as;
  o_region*   region;
  t_uint32    i;
  ...
  for (i = 0; i < mips_number_entry(); i++)
  {
    ...
    /* get the i-th entry */
    if (mips_get_tlb(i, &vaddr, &id) == ERROR_NONE)
    {
...
Un parcours de liste est une longue opération. Il faudrait utiliser
une autre structure de données pour stocker et chercher les regions
plus rapidement en fonction de l'adresse virtuelle qui a causé
l'exception.
