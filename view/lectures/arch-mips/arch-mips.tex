...
%% updated       julien quintard   [mon jan  2 09:22:10 2006]
%%
...
\pgfdeclareimage[interpolate=true,width=185pt,height=124pt]
...
\begin{frame}[containsverbatim]
  ...
  \begin{verbatim}
    cycles per access = 1 x (4 Kb / 4 Gb) +
                        10 x ((4 Mb - 4 Kb) / 4Gb) +
                        10^6 x ((4 Gb - 4 Mb) / 4Gb)
                     ~= 10^-6 +
                        10^-3 - 10^-6 +
                        1 - 10^-3
                     ~= 999 000
...
\begin{frame}[containsverbatim]
  ...
  \begin{verbatim}
    cycles per access = 1 x 0.90 +
                        8 x 10 x 0.10 x (1 - 10^-5) +
                        1024 x 10^6 x 10^-5
                     ~= 10200
  \end{verbatim}
\end{frame}
...
\begin{frame}[containsverbatim]
  ...
  \begin{verbatim}
    cycles per access = 1 x 0.97 +
                        (9 + (8 x 1)) x 0.03 x (1 - 10^-5) +
                        (999900 + (1024 x 100)) x 10^-5
                     ~= 10
  \end{verbatim}
\end{frame}
...
\begin{frame}
  ...
  Let's notice that a directory entry like a data entry can takes exactly
...
\begin{frame}
  ...
  per block depending on the associative degree.

  \nl

  Let's study this algorithm. At the beginning, every LRU bit is set to zero.

  \begin{enumerate}
    \item
      If an access is made to a block, in reading or writing,
      its LRU bit is set to one.
    \item
      If every LRU bits of a set are set to one, then the LRU bits are
      reinitialised to zero.
  \end{enumerate}

  Then, the pseudo LRU algorithm just iteratively looks for an entry with
  a LRU bit not set.

  \nl

  We called this algorithm pseudo LRU because nothing permit the cache
  manager to distinguish two block with the LRU bit not set.
\end{frame}
...
\begin{frame}
  ...
  Of courses these types of cache are theoritical since it takes much
  time to perform an iterative lookup in the entire cache and the
  hardware become too complex with many multiplexers.
\end{frame}
...
\begin{frame}
  ...
  \begin{itemize}[<+->]
    ...
    \item
      ...
      the cache entry if any. Otherwise, the memory is updated.
...
\begin{frame}
  ...
  Of course, sending a Invalidation Signal takes time because the bus needs
...
\begin{frame}
  ...
  When P1 will want to access the block B, it will be from the main memory.

  \nl

  Of course, when a microprocessor receive a Invalidation Signal, it
...
\begin{frame}
  ...
  So a Delay Buffer was introduced to optimise the writings.

  \nl

  Everytime a block is overwritten, the block is pushed in the Delay Buffer.

  \nl

  Then the cache automata's role is now to get a block from the Delay Buffer
...
\begin{frame}
  ...
  \begin{itemize}[<+->]
	...
	\item
	  With Delay Buffer.
	\item
	  Without Delay Buffer. The microprocessor is blocked until the
    ...
    \item
      ...
      \begin{itemize}
	\item
	  Miss. The block is loaded in cache then the block is overwritten
	  in cache.
	\item
	  Hit.
      \end{itemize}
...
\begin{frame}[containsverbatim]
  ...
  \begin{verbatim}
    RP = penalty / reading   x   reading number / instruction
       =        9            x  (0.2 x 2  +  0.1 x 1  +  0.7 x 1)
       =        9            x               1.2
       =                 10.8
  \end{verbatim}
\end{frame}
...
\begin{frame}[containsverbatim]
  ...
  \begin{verbatim}
    WP = penalty / writing   x   writing number / instruction
       =        3            x  (0.2 x 0  +  0.1 x 1  +  0.7 x 0)
       =        3            x               0.1
       =                 0.3
  \end{verbatim}
\end{frame}
...
\begin{frame}
  ...
  the data is not loaded in cache. There is no delay buffer for writings.
\end{frame}
...
\begin{frame}[containsverbatim]
  ...
  \begin{verbatim}
    sets = cache size / block size / associative degree
         = 16 Kb / 16 b / 2
         = 512
  \end{verbatim}
...
\begin{frame}
  ...
  To respect to Temporal Locality.
\end{frame}
...
\begin{frame}
  ...
  \textit{From the perfect system, what are the CPI increase for
...
\begin{frame}
  ...
  The Write Penalty is the same than before since there is no delay
  buffer for writings.

  \nl

  The Write Penalty is so the same than if there is no delay buffer:
...
XXX changer les ^6 en \exp{6}

XXX transformer les formules en utilisant les notations de maths.
