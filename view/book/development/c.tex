...
% updated       julien quintard   [tue may 20 17:03:43 2008]
%
...
\begin{itemize}
  \item
    Manager types are prefixed by \code{m\_};
  \item
    kaneton object types are prefixed by \code{o\_};
  \item
    Dispatch interfaces are prefixed by \code{d\_};
  \item
    Architecture-dependent types are prefixed by \code{a\_};
  \item
    Platform-dependent types are prefixed by \code{p\_};
  \item
    kaneton identifiers are prefixed by \code{i\_}.
\end{itemize}

Note that group prefixes can be combined in several ways. For instance,
the prefix \code{ao\_} represents the architecture-dependent part of
an object.
