...
% updated       julien quintard   [mon may 28 20:34:26 2007]
%
...
than encouraged as it leads to a very readable source code.
...
put the open brace at the end of the constrol structure line.
...
of whitespaces. This amount is fixed, in the kaneton project context, to
\textit{2} spaces. Note that the \textit{Emacs} default identation comply
to this rule.
  ...
  if (type == TYPE_SET)
    {
...

      {
        ...
        /*
         * this inner block is not indented from the parent block.
         */
      }
    }
\end{verbatim}
...
Extern declarations must not be contained in functions but rather in the
global scope.
...
\begin{verbatim}
  t_error                 obscure_example(i_timer                      id,
                                          i_as                         as)
  {
    int                   i;
...
There exist exception the the above rules. For more information, please
refer the to section about control structures.

Statements keywords must be followed by a single whitespace, except
...
Statements keywords which takes an argument must enclose the argument
