...
% updated       julien quintard   [wed nov 24 09:10:39 2010]
%
...
\begin{itemize}
  ...
  \item
    ...
    points to tests according to a \name{point} file;
  \item
    These grade points are then compiled into a final grade which is
    normalised according to a \name{reference} maximal grade, $20$ for
    instance;
  \item
...
\begin{itemize}
  ...
  \item
    The \location{test/hooks/} directory which contains the scripts for
  ...
  \item
    ...
    requests;
  \item
    The \location{test/points/} directory which contains files specifying
...
the \name{test} module in his environment profile and then move to the
\location{test/tests/} and launch a compilation process through the following
command:
...
Note that should the kernel crash or hang, the whole test suite whould be
