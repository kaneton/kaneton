...
% updated       julien quintard   [tue may 29 23:27:58 2007]
%
...
Although the profile directories can contain any number of configuration files
it is not the case of the \textit{user} profile directory. Indeed, there must
be a single configuration file named with

XXX conseille d'avoir un seul fichier a chaque fois meme si possible autrement

XXX -state type variable
...
\function{env\_content}{(\argument{file},
                         \argument{options})}
         {
	   This function returns the content of the file \argument{file}.
	 }
