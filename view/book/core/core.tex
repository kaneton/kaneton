...
\begin{itemize}
\function{message\_send}{(i\_task \argument{caller},
			  i\_node \argument{destination},
			  t\_type \argument{type},
			  t\_vaddr \argument{message})}
	 {
	   This function performs an Asynchronous send.
	   This function is Blocking.

	   \item \argument{caller}

	     Identifier of the sending task.

	   \item \argument{destination}
	   ...
	   \item \argument{message}

	     Virtual adress of the message to send in the
	     \argument{caller}'s adress space
	 }

\function{message\_transmit}{(i\_task \argument{caller},
			     i\_node \argument{destination},
			     t\_type \argument{type},
			     t\_vaddr \argument{message})}
	 {
...
\function{message\_throw}{(i\_task \argument{caller},
			   i\_node \argument{destination},
			   t\_type \argument{type},
			   t\_vaddr \argument{message},
			   t\_message\_request* \argument{request})}
	 {
	   ...
	   \item \argument{request}

	     Handle to the message transmission.
	 }

\function{message\_receive}{(i\_task \argument{caller},
			     t\_type \argument{type},
			     t\_vaddr \argument{message},
...
\function{message\_poll}{(i\_task \argument{caller},
			  t\_type \argument{type},
			  t\_vaddr \argument{message},
			  i\_node* \argument{sender})}
	 {
...
\function{message\_grab}{(i\_task \argument{caller},
			  t\_type \argument{type},
			  t\_vaddr \argument{message},
			  i\_node* \argument{sender},
			  t\_message\_request* \argument{request})}
...
\function{message\_register}{(i\_task \argument{task},
			      ...
			      t\_vsize \argument{size})}
	 {
	   Register for \argument{task} the type of message
	   \argument{type} which is \argument{size} bytes long.
	 }

\function{message\_wait}{(t\_message\_request \argument{request})}
	 {
	   Blocks the calling task until the message request described by
	   \argument{request} is fully completed.
	 }


\function{message\_state}{(t\_message\_request \argument{request})}
	 {
	   Returns an error if the message request described by
	   \argument{request} is not fully completed.
	 }
