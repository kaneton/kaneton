...
A tag (\textit{t\_tag}) is a 32-bit integer which permit to mark a message
(as coming from a a specific task for example). Each tag is associated with
a message size.

The special value \textit{MESSAGE\_TAG\_ANY} permits to send an untagged
message or to receive a message marked with any tag.
...
\function{message\_register}{(i\_task \argument{task},
			      t\_tag \argument{tag},
			      t\_vsize \argument{msg\_sz})}
	 {
	    
	 }

\function{message\_size}{(i\_task \argument{task},
			  t\_tag \argument{tag},
			  t\_vsize* \argument{size})}
	 {
...
\function{message\_send}{(i\_node \argument{node},
                          ...
                          void* \argument{data})}
	 {
	   This blocking function sends the message described by
...
\function{message\_throw}{(i\_node \argument{node},
                           ...
                           void* \argument{data})}
	 {
	   This non-blocking function sends the message described by
	   \argument{data} to the node \argument{node}.
