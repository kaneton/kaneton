...
\function{message\_throw}{(i\_task \argument{caller},
			   ...
			   t\_type \argument{type},
			   t\_vaddr \argument{msg},
			   t\_message\_request* \argument{rq})}
	 {
	   This function performs an Asynchronous send.
...
\function{message\_register}{(i\_task \argument{task},
			      ...
			      t\_vsize \argument{msgsz})}
	 {
	   ...
	   \argument{type} which is \argument{msgsz} bytes long.
	 }

\function{message\_wait}{(t\_message\_request \argument{rq})}
	 {
	   Blocks the calling task until the message request described by
	   \argument{rq} is fully completed.
	 }


\function{message\_state}{(t\_message\_request \argument{rq})}
	 {
	   Returns an error if the message request described by
	   \argument{rq} is not fully completed.
	 }
