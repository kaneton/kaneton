...
FUNC(ramdac)
{
    ...
    const int ybase = 100 - 32;
    const int xbase = 160 - 24;

    for (i = 0; i < MAX_HEIGHT; i++)
      memcpy(&framebuffer[(ybase + i) * 320 + xbase], &offscreen[i * 320], MAX_WIDTH);

    if (pos < 11)
      draw_text("CHICHE", xbase, ybase + MAX_HEIGHT + 10, 255, 0);
    else if (pos < 22)
      draw_text(" DONNE ", xbase - 4, ybase + MAX_HEIGHT + 10, 255, 0);
    else if (pos < 33)
      draw_text(" NOUS ", xbase, ybase + MAX_HEIGHT + 10, 255, 0);
    else
      draw_text(" TOUT ", xbase, ybase + MAX_HEIGHT + 10, 255, 0);
