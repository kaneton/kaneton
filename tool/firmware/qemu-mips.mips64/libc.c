 ...
 * license       kaneton
 *
 * file          /home/enguerrand/...on/tool/firmware/qemu-mips.mips64/libc.c
 *
 * created       matthieu bucchianeri   [thu jun 28 16:14:31 2007]
 * updated       enguerrand raymond   [mon apr 27 12:24:49 2009]
 */
				      ...
				      int			len2)
{
  ...
  if (!(flags & PRINTF_DOT))
	...
	padlen = -padlen;
      ...
      for (; padlen > 0; padlen--)
	written += firmware_cons_print_char(' ');
    }

  for (i = 0; (string[i] != 0) && (i < (len1 != -1 ? len1 : i + 1)); i++)
    written += firmware_cons_print_char(string[i]);
				    ...
				    int				len2)
{
  ...
  if ((flags & PRINTF_ZPAD) && (padlen > 0))
    {
      if (sign)
	{
	  written += firmware_cons_print_char(sign);
	  sign = 0;
	  padlen--;
	}

      for (; padlen > 0; padlen--)
	written += firmware_cons_print_char('0');
    }
  ...
  for (i--; i >= 0; i--)
    written += firmware_cons_print_char(convert[i]);
	...
	case '%':
	  ...
	  _next_char_:
	      ...
	      case 'c':
		{
		  ...
		  written += firmware_cons_print_char(charvalue);

		  break;
		}
	      case '%':
		{
		  written += firmware_cons_print_char(c);
		...
		{
		  written += firmware_cons_print_char('%');
		  written += firmware_cons_print_char(fmt[i]);
		}
	      }
